...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        ...
        self.no = nc + 5  # number of outputs per anchor  每个位置都预测80个类（coco）+ 4个位置坐标xywh + 1个confidence score
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):    # 对三个feature map分别进行处理
            ...
            if not self.training:  # inference
                ...
                y = x[i].sigmoid()     #y 为预测的偏移量
                #关于xy的公式，先sigmoid限制到0-1，再乘2变成0-2，然后减0.5变为-0.5-1.5，为什么这么做是和build_target这个函数有关的。
                #TODO 为什么要有回归方程
                if self.inplace:
                    y[..., 0:2] = (y[..., 0:2] * 2 + self.grid[i]) * self.stride[i]  # xy   [-0.5,3.5]
                ...
                z.append(y.view(bs, -1, self.no))   ## z是一个tensor list 三个元素 分别是[1, 19200, 25] [1, 4800, 25] [1, 1200, 25]
    ...
    def _make_grid(self, nx=20, ny=20, i=0):
        ...
        shape = 1, self.na, ny, nx, 2  # grid shape    ‘2’表示xy轴的偏移量
        y, x = torch.arange(ny, device=d, dtype=t), torch.arange(nx, device=d, dtype=t)
        if check_version(torch.__version__, '1.10.0'):  # torch>=1.10.0 meshgrid workaround for torch>=0.7 compatibility
            yv, xv = torch.meshgrid(y, x, indexing='ij')
        else:
            yv, xv = torch.meshgrid(y, x)
            #torch.meshgrid 的功能是生成网格，可以用于生成坐标。函数输入两个数据类型相同的一维张量，两个输出张量的行数为第一个输入张量的元素个数，
            # 列数为第二个输入张量的元素个数。其中第一个输出张量填充第一个输入张量中的元素，各行元素相同；第二个输出张量填充第二个输入张量中的元素各列元素相同。
            #torch.stack  沿一个新维度对输入张量序列进行连接，序列中所有张量应为相同形状；
            #expand 扩展到指定维度
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        # depth gain 控制深度number  如v5s: n*0.33   n: 当前模块的次数(间接控制深度)
        ...
        if m in (Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
            ...
            if c2 != no:  # if not output
                # width gain 控制宽度args  如v5s: c2*0.5  c2: 当前层的最终输出的channel数(间接控制宽度)
        ...
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module 模型深度
        ...
        # 把当前层的输出channel数加入ch
        ch.append(c2)
    return nn.Sequential(*layers), sorted(save)    #nn.Sequential   构建一个有序的神经网络容器
