...
class DWConv(Conv):
    # Depth-wise convolution class     Depthwise convolution 深度可分离卷积
...
class Bottleneck(nn.Module):
    # Standard bottleneck   bottleneck网络不改变图像尺寸
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        self.m = nn.Sequential(*(Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)))
        #Bottleneck类似ResNet的残差结构
        #C3这里除了用Bottleneck，还可以用什么？
    def forward(self, x):
...
class Focus(nn.Module):
    ...
    # 设计思路：理论上从高分辨率图像中，周期性的抽出像素点重构到低分辨率图像中，即将图像相邻的四个位置进行堆叠，
    # 聚焦wh维度信息到c通道空，提高每个点感受野，并减少原始信息的丢失。这个组件并不是为了增加网络的精度的，而是为了减少计算量，增加速度。
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
...
class Contract(nn.Module):
    #这两个函数是用于改变feature map的维度，并不是很常用。Contract函数改变输入特征的shape，
    # 将feature map的w和h维度(缩小)的数据收缩到channel维度上(放大)。如：x(1,64,80,80) to x(1,256,40,40)。
...
class Concat(nn.Module):
    #这个函数是讲自身（a list of tensors）按照某个维度进行concat，常用来合并前后两个feature map
...
class Detections:
    ...
    def save(self, labels=True, save_dir='runs/detect/yolov5s'):
        save_dir = increment_path(save_dir, exist_ok=save_dir != 'runs/detect/yolov5s', mkdir=True)  # increment save_dir
        self.display(save=True, labels=labels, save_dir=save_dir)  # save results

    def crop(self, save=True, save_dir='runs/detect/yolov5s'):
        save_dir = increment_path(save_dir, exist_ok=save_dir != 'runs/detect/yolov5s', mkdir=True) if save else None
