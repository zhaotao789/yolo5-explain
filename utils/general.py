...
def labels_to_class_weights(labels, nc=80):
    ...
    weights = np.bincount(classes, minlength=nc)  # occurrences per class    统计输入的数组中的每个值在非负整数数组中出现的次数
...
def non_max_suppression(prediction,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Compute conf    每一类的分类置信度=该类类别置信度*边框置信度
        ##TODO 类别置信度和样本量有关，样本量越大，置信度越高
        x[:, 5:] *= x[:, 4:5]  # conf = obj_conf * cls_conf    计算每个box框的综合置信度conf：conf = obj_conf * max(class_1_prob ,class_2_prob)，
        ...
        if multi_label:
            #nonzero 返回非0元素的索引
            #as_tuple = False (Boolean) - 如果 as_tuple 为 False (默认值)，返回一个包含输入张量中非零元素的索引的 2D 张量；
            # 如果 as_tuple 为 True，对于输入张量的每一个维度都返回一个 1D 张量，1D 张量中的元素是沿着该维度上非零元素的索引；
            i, j = (x[:, 5:] > conf_thres).nonzero(as_tuple=False).T
            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)     #x[i, j + 5, None], j[:, None]    None:将一维转为二维
...
def increment_path(path, exist_ok=False, sep='', mkdir=False):
    # Increment file or directory path, i.e. runs/yolov5s --> runs/yolov5s{sep}2, runs/yolov5s{sep}3, ... etc.
